...
start_v_t      <- if(is.na(beta_0_st)) {unname(c(sigma_u,sigma_v,a_start,beta_hat))}    else{unname(c(sigma_u,sigma_v,a_start,beta_0,beta_hat)) }
...
start_v_thn    <- if(is.na(beta_0_st)) {unname(c(sigma_v,sigma_u,a_start,beta_hat))}  else{unname(c(sigma_v,sigma_u,a_start,beta_0,beta_hat)) }
...
start_v_nr     <- if(!is.null(.nr_mom)) {.nr_mom}
                  else if(is.na(beta_0_st)) {unname(c(sigma_v,sigma_u,beta_hat))}      else{unname(c(sigma_v,sigma_u,beta_0,beta_hat)) }
...
if(model_name %in% c("NLN","NW")){
  ...
  start_v        <- if(is.na(beta_0_st)) {unname(c(sv_st,su_st,th_st,beta_hat))} else {unname(c(sv_st,su_st,th_st,beta_0,beta_hat))}
  out            <- matrix(0,nrow = 3,ncol = length(start_v))
  colnames(out)  <- c("sigv","sigu", if(model_name == "NLN") "mu" else "k",
                      c(names(plm_lm$coefficients)))
  lower_bob      <- c(rep(.Machine$double.eps,2),
...
if(model_name %in% c("NU","NGE")){
  ...
  start_v        <- if(is.na(beta_0_st)) {unname(c(sv_st,su_st,beta_hat))} else {unname(c(sv_st,su_st,beta_0,beta_hat))}
  out            <- matrix(0,nrow = 3,ncol = length(start_v))
  colnames(out)  <- c("sigv", if(model_name == "NU") "theta" else "sigu",
                      c(names(plm_lm$coefficients)))
  lower_bob      <- c(rep(.Machine$double.eps,2),rep(-Inf,n_x_vars))}
if(model_name=="THT"){
  start_v        <- start_v_t
...
if(model_name=="tHN"){
  ...
  sv_st          <- max(0.5*s_eps, 1e-3)
  su_st          <- max(s_eps,     1e-3)
  start_v_thn    <- if(is.na(beta_0_st)) {unname(c(sv_st,su_st,a_start,beta_hat))} else {unname(c(sv_st,su_st,a_start,beta_0,beta_hat))}
  start_v        <- start_v_thn
  out            <- matrix(0,nrow = 3,ncol = length(start_v))
...
if(!is.null(collinear_chk) && length(collinear_chk$between_drop)){
  ...
  keep_terms <- setdiff(attr(stats::terms(formula_x), "term.labels"),
...
beta_hat_raw <- if(isTRUE(intercept==0)){plm(formula_start ,data,effect = "individual")$coefficients} else{plm_gtre$coefficients[-c(1)]}
...
if(model_name %in% c("NHN","NE","NR","NTN","NU","NGE")){                 lower1 <- c(rep(.0000001,2),   start_v[-c(1:2)] - differ )}
...
if(model_name =="THT"){                                                  lower1 <- c(rep(.0000001,2), 2.05, start_v[-c(1:3)] - differ )}
if(model_name =="tHN"){                                                  lower1 <- c(rep(.0000001,2), 2.05, start_v[-c(1:3)] - differ )}
if(model_name %in% c("NG","NNAK","NW")){                                 lower1 <- c(rep(.0000001,3),   start_v[-c(1:3)] - differ )}
## NLN's third parameter is a meanlog and is genuinely unbounded below.
if(model_name == "NLN"){                                                 lower1 <- c(rep(.0000001,2), start_v[3]-differ, start_v[-c(1:3)] - differ )}
