...
fn = function(p){
  ...
  if(is.null(D)){return(.Machine$double.xmax)}
...
metrics.hn <- function(p, e=NULL, y=Y, xx=data_i_vars, zu=data_z_vars, zw=data_zp_vars){
  ...
  .biv2 <- function(xvec, yvec, rhovec){
    ...
    xvec <- rep_len(xvec, n); yvec <- rep_len(yvec, n); rhovec <- rep_len(rhovec, n)
  ...
  list(M1=M1, M2=M2, M5=M5, M6=M6, M7=M7, M10=M10, Eew.cond=Eew.cond, Eemw.cond=Eemw.cond,
       Eeu.cond=Eeu.cond, Eemu.cond=Eemu.cond, Eewmu.cond=Eewmu.cond, Eeumw.cond=Eeumw.cond)
}
...
fn = function(p){
  ...
  if(any(is.na(ss))){return(.Machine$double.xmax)}
  if(is.null(ss)){return(.Machine$double.xmax)}
...
else{stop(paste0("model_name '", model_name, "' is a recognized choice for ttsfm() but has no implementation branch. ",
                  "Valid, implemented options are: 'TTNE', 'TTHN', 'TTNLS'."), call. = FALSE)}}
