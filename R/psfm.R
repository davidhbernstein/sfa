psfm <- function(formula, 
        ...
        opg_se <- tryCatch({
          ...
          fn_vec <- function(x){
            ...
            ll_i_vec <- function(ii){
              ...
              sigma_h_fun <- if(n_zp_vars > 1){
                eta_h_i <- as.numeric(data_zp_vars[[ii]] %*% delta_p)
                if(any(!is.finite(eta_h_i))) return((.SFA_CONSTANTS$MAX_VALUE)^0.1)
                eta_h_i <- pmin(pmax(eta_h_i, -eta_bound), eta_bound)
                mean(sqrt(exp(eta_h_i)))
              } else {
                eta_h_i <- x[deltap_end]
              ...
              sigma_u_fun <- if(n_z_vars > 1){
                eta_u_i <- as.numeric(data_z_vars[[ii]] %*% delta)
                if(any(!is.finite(eta_u_i))) return((.SFA_CONSTANTS$MAX_VALUE)^0.1)
                eta_u_i <- pmin(pmax(eta_u_i, -eta_bound), eta_bound)
                sqrt(exp(eta_u_i))
              } else {
                eta_u_i <- x[delta_end]
              ...
              sim_terms <- (2 / sigma_fun) *
                dnorm(eps_ii / sigma_fun) *
                pmax(pnorm(-eps_ii * lamb_fun / sigma_fun),
                     .SFA_CONSTANTS$MIN_POSITIVE)
  ...
  if(model_name ==     "TFE")        {
    ...
    like.tfe  <-  function(x){
      ...
      if(!is.finite(lambda_eff)) return(1e12)
  ...
  if(model_name ==     "GTRE_SEQ1")  {
    ...
    gs_se <- function(fit){
      p <- fit$par[1:2]
      if(isTRUE(any(fit$hessian==0))) return(c(NA_real_, NA_real_))
      ...
      if(is.null(jac) || is.null(vc)) c(NA_real_, NA_real_) else
  ...
  if(model_name ==     "SSFE")       {
    ...
    results <- list(t(out), End.Time, model_name, formula, data, alpha_hat, u_hat, exp_u_hat,
  ...
  if(model_name %in% c("PL80","BC92"))       {
    ...
    differ <- 2
    Opt.Psoptim <- opt.psoptim(fn=like.pl, start_v, lower.psoptim=c(rep(.SFA_CONSTANTS$MIN_POSITIVE,2), start_v[-c(1:2)]-differ),
                               rand.psoptim = rand.psoptim,
    ...
    Opt.Optim <- opt.optim(fn=like.pl, start_v=start_v, lower.optim=lower_pl, upper.optim=rep(Inf, n_par),
    ...
    st_err <- if(isTRUE(any(opt$hessian==0)) | optHessian==FALSE){
      ...
      if(is.null(jac) || is.null(vc)) rep(NA, length(par_names)) else
        suppressWarnings(sqrt(pmax(diag(jac %*% vc %*% t(jac)), 0)))
    }
    out[2,] <- st_err
    out[3,] <- out[1,]/out[2,]
    ...
    results <- list(t(out), opt, End.Time, model_name, formula, data, exp_u_hat,
                     out["par",], out["st_err",], out["t-val",], call)
    class(results) <- "sfareg"
    names(results) <- c("out","opt","total_time","model_name","formula","data","exp_u_hat",
                         "coefficients","std.errors","t.values","call")
    return(results)}
