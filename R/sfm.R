...
if(estimator == "cols" && robust != "mle")
  stop("`robust` applies to the maximum-likelihood estimator only. ",
       ...
       "robustify; call it with robust = \"mle\".", call. = FALSE)

.validate_sfa_call(formula, data, "sfm",
                   maxit = list(maxit.bobyqa = maxit.bobyqa, maxit.nlminb = maxit.nlminb,
                                maxit.psoptim = maxit.psoptim, maxit.optim = maxit.optim),
                   flags = list(optHessian = optHessian, PSopt = PSopt,
...
if(estimator == "cols"){
  ...
  if(is.numeric(cols_boot) && length(cols_boot) == 1L && cols_boot >= 1){
    ...
    for(bb in seq_len(as.integer(cols_boot))){
      ...
      bfit <- tryCatch(.cols_fit(Yc[idx], Xc[idx, , drop = FALSE], model_name,
                                 intercept_col = icol), error = function(e) NULL)
      if(!is.null(bfit))
  ...
  if(CF$sigma_u > 0){
    s2u <- CF$sigma_u^2; s2v <- CF$sigma_v^2
    exp_u_hat <- .te_battese_coelli(mu_star    = -eps_c*s2u/(s2u + s2v),
                                    sigma_star = CF$sigma_u*CF$sigma_v/sqrt(s2u + s2v))
  } else exp_u_hat <- rep(NA_real_, length(eps_c))

  End.Time <- end.time(Start.Time)
  results <- list(t(out), NULL, End.Time, NULL, model_name, formula, exp_u_hat,
...
if(model_name %in% c("NLN","NW")){
  ...
  .auto_nsim <- function(n) if(model_name == "NLN")
                              max(200L, as.integer(ceiling(3*sqrt(n))))
                            else
                              max(400L, as.integer(ceiling(8*sqrt(n))))
  Nsim <- if(identical(Nsim, "auto")) .auto_nsim(n_obs)
          else max(as.integer(Nsim), 10L)
  ...
  FiMat  <- matrix(pmin(pmax(hseq, 1e-6), 1 - 1e-6), nrow = n_obs, ncol = Nsim,
                   byrow = TRUE)
...
if(model_name %in% c("NHN","NE","NR","NU","NGE")){           x_x_vec <- x[3:as.numeric(n_x_vars + 2)]}
if(model_name %in% c("THT","NTN","NLN","NW","tHN","NG","NNAK")){ x_x_vec <- x[4:as.numeric(n_x_vars + 3)]}
...
if(model_name %in% c("NE_Z","NHN_Z")){data_z_vars <- as.matrix(data.frame(subset(data,select = z_vars)))
      ...
      if(model_name %in% c("NLN","NW")){
        ...
        if(model_name == "NLN"){
          ...
          if(any(!is.finite(u_draw))) return(1e12)
      ...
      if(model_name=="tHN"){
        ...
        like  <- if(!is.finite(sig_v) || !is.finite(sig_u) || !is.finite(nu) ||
                 ...
                 } else .log_d_thn(eps, sig_v, sig_u, nu) }
...
if(model_name == "tHN"){
  .thn_try <- function(sv){
    ...
    o  <- tryCatch(suppressWarnings(
            stats::optim(sv, like.fn, method = "L-BFGS-B", lower = lower_bob,
                         control = list(maxit = 300))), error = function(e) NULL)
    if(is.null(o) || !is.finite(o$value)) NULL else o
  }
  .cand <- list(start_v,                                             # OLS/moment start
                ...
                replace(start_v, 2:3, c(start_v[2]*8.0,  2.5)))      # extreme one-sided
  ...
  if(length(.fits)){
    ...
    thn_starts <- list(n_tried = length(.cand), n_converged = length(.fits),
...
if(model_name == "NG" && isFALSE(is.numeric(start_val))){
  ...
  .obj  <- vapply(.cand, function(z)
                    tryCatch({v <- like.fn(z); if(is.finite(v)) v else Inf},
  ...
  if(any(is.finite(.obj))){
    ...
    .fits <- Filter(Negate(is.null), lapply(.ord, function(i)
      tryCatch(suppressWarnings(
        stats::optim(.cand[[i]], like.fn, method = "L-BFGS-B",
    ...
    if(length(.fits)){
      ...
      ng_starts <- list(n_tried = length(.cand), n_polished = length(.fits),
...
Opt.Nlminb  <- opt.nlminb(fn=like.fn, start_v=start_v, lower.nlminb=lower_bob,
...
if(thn_nu_unidentified)
  warning("sfm(model_name = \"tHN\"): nu converged to ", signif(nu, 4),
          ...
          "other parameters are unaffected. See ?sfm.", call. = FALSE)
...
if(thn_sigma_u_at_bound)
  warning("sfm(model_name = \"tHN\"): sigma_u collapsed to the boundary (",
          ...
          "the efficiency scores as uninformative. See ?sfm.", call. = FALSE)}
...
if(model_name == "NG"){
results <- list(t(out),c(opt),End.Time,start_v,model_name,formula, exp_u_hat, ng_starts,
                out["par",], out["st_err",], out["t-val",],call )
class(results) <- "sfareg"
names(results)  <- c("out","opt","total_time","start_v","model_name","formula","exp_u_hat","ng_starts",
                     "coefficients", "std.errors", "t.values","call")}

## NE/NTN additionally return u_hat (the JLMS E[u|e] point predictor)
## alongside exp_u_hat.
if(model_name %in%  c("NE","NTN","NU","NGE","NLN","NW") ){
results <- list(t(out),c(opt),End.Time,start_v,model_name,formula, exp_u_hat, u_hat,
                out["par",], out["st_err",], out["t-val",],call )
class(results) <- "sfareg"
names(results)  <- c("out","opt","total_time","start_v","model_name","formula","exp_u_hat","u_hat",
                     "coefficients", "std.errors", "t.values","call")}
...
if(model_name == "THT"){
results <- list(t(out),c(opt),End.Time,start_v,model_name,formula, exp_u_hat, u_hat, sd_exp_u_hat,
                out["par",], out["st_err",], out["t-val",],call )
class(results) <- "sfareg"
names(results)  <- c("out","opt","total_time","start_v","model_name","formula","exp_u_hat","u_hat","sd_exp_u_hat",
                     "coefficients", "std.errors", "t.values","call")}
